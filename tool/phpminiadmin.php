...
<a href="?q=show+databases">Veritabaný</a>: <select name="db" onChange="frefresh()"><option value='*'> - select/refresh -</option><option value=''> - show all -</option><?php echo get_db_select($dbn)?></select>
...
<?php } ?>
 | <a href="?showcfg=1">Ayarlar</a>
<?php } ?>
<?php if ($GLOBALS['ACCESS_PWD']){?> | <a href="?logoff=1">Logoff</a> <?php }?>
 | <a href="?phpinfo=1">phpinfo</a>  | <a href="?module=1">T.E Modül Düzenleme Aracý</a>
</div>
...
<div align="right">
<small>&copy; 2004-2009 Oleg Savchuk <a href="http://phpminiadmin.sourceforge.net/" target="_blank"><b>phpMiniAdmin <?php echo $VERSION?></b></a></small>
</div>
</body></html>
<?php
}
...
?>
<center>
<h3>Veritabaný Yönetim Paneli</h3>
<div style="width:400px;border:1px solid #999999;background-color:#eeeeee">
Parola: <input type="password" name="pwd" value="">
...
?>
<center>
<h3>Veritabaný Baðlantý Ayarlarý</h3>
<div class="frm">
...
function save_module(){
 global $DB,$err_msg,$self,$PHP_SELF,$module_directory;
 $module = $_REQUEST['modulename'];
 $filename = $module_directory.$module;
 $somecontent = $_REQUEST['source'];
 print_header();
?>
<center>
<?php
...
echo '<div class="err"><font color="#000000">'.$module.'</font> dosyasý açýlamadý.</div>';
}
if (fwrite($handle, $somecontent) === FALSE) {
echo '<div class="err"><font color="#000000">'.$module.'</font> dosyasýna yazýlamadý.</div>';
}
echo '<b><font color="red">'.$module.'</font> dosyasýna kodlar baþarýlý biçimde yazýldý.</b>';
fclose($handle);
} else {
echo '<div class="err"><font color="#000000">'.$module.'</font> dosyasý için yazma izni yok.</div>';
}
chmod($filename, 0444);
?>
<div class="dot"></div>
<h3>T.E Modül Listesi</h3>
<div class="frm">
<center>
<select name="modules">
<?php
...
foreach($directory_array as $modules)
echo '<option value="'.$modules.'">'.$modules.'</option>';
?>
</select>
<input type="hidden" name="selectmodule" value="1">
<input type="submit" value=" Aç "><input type="button" value=" Ýptal " onclick="window.location='<?php echo $self?>'">
</center>
</div>
</center>
<?php
...
function select_module(){
 ...
 $myFile = $module_directory.$_REQUEST['modules'];
 $fh = fopen($myFile, 'r');
...
?>
<center>
<h3><?php echo $module;?> T.E Modülü</h3>
<div class="dot" style="padding:0 0 5px 20px">
Düzenlemeyi yaptýktan sonra kaydet e týklayýn.<br />
<textarea name="source" cols="70" rows="20" style="width:98%"><?php echo $theData?></textarea><br/>
<input type="hidden" name="modulename" value="<?php echo $module;?>">
<input type="hidden" name="savemodule" value="1"> <input type="submit" value=" Kaydet ">&nbsp;&nbsp;
<input type="button" value=" Ýptal " onclick="window.location='<?php echo $self?>?module=1'">
</div>
</center>
<?php
 print_footer();
...
global $DB;
 $v=$_REQUEST['v'];
 $_SESSION['DB']=$DB;
 $_SESSION['DB']['chset']=$v['chset'];
 unset($_SESSION['sql_sd']);

 if ($_REQUEST['rmb']){
    $tm=time()+60*60*24*30;
    setcookie("conn[db]",$DB['db'],$tm);  //$v['db']
    setcookie("conn[user]",$DB['user'],$tm);  //$v['user']
    setcookie("conn[pwd]",$DB['pwd'],$tm);  //$v['pwd']
    setcookie("conn[host]",$DB['host'],$tm);  //$v['host']
    setcookie("conn[port]",$DB['port'],$tm);  //$v['port']
