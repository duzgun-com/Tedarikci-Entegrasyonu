...
<body>
<p><strong>Tedarikçi Entegrasyonu Modül Oluşturma Sihirbazı</strong></p>
<p>Eğer tedarikçiniz size evrensel XML standartlarında data akışı sağlıyorsa bu xml bilgisinde bulunan sadece bir ürünün tagını sahte bilgilerle düzenleyip(bu okumak istediğiniz xmlin yapısını analiz etmenizi sağlamaktadır.)aşağıdaki metin kutusuna yazarak otomatik Tedarikçi Entegrasyonu Modülünü oluşturabilirsiniz. XML yapısını yazarken kesinlikle gerçek bilgileri kullanmayınız. XML içerikleri tedarikçi ile bayi arasıda telif hakları korunan datalardır.
</p>
<p>Kullanıdığınız bu formadaki bilgiler hiçbir şekilde kayıt altına alınmamaktadır. Modül sihirbazı ile oluşturduğunuz modüllerde hatalar olabilir,olmayabilirde. Güvendiğiniz herhangi bir php yazılımcısından destek alarak modül yazım klavuzları eşliğinde modülü düzenleyebilirsiniz. Bu modül sihirbazı php yazılımcılarının işlerini kolaylaştırmak için yazılmıştır.<?php /* Tedarikçi Entegrasyonu yazılımı için herhangi bir tedarikçi için modül oluştururken sitemizde aşağıdaki bulunan formu kullanmak yerine bu modül oluşturma sihirbazının kaynak kodlarını indirerek kendi bilgisayarınızda çalıştırabilirsiniz. <a href="#">Tedarikçi entegrasyonu yazılımında kullanmak üzere modül oluşturma sihirbazı kaynak kodlarını indirmek için tıklayın.</a>*/ ?></p>

<p>Aşağıdaki formda örnek bir XML yapısı vardır. Siz bu yapıyı değiştirerek her tür kalıba uygun modül geliştirebilirsiniz.</p>
<form id="form1" name="form1" method="post" action="module_wizard.php?process=1">
<textarea name="metinkutusu" id="metinkutusu" cols="80" rows="15">&lt;?xml version=&quot;1.0&quot; encoding=&quot;ISO-8859-9&quot;?&gt;
&lt;urun_listesi&gt;
&lt;urun&gt;
...
&lt;resim&gt;&lt;/resim&gt;
&lt;/urun&gt;
&lt;/urun_listesi&gt;</textarea>
<br />
<br />
<input type="submit" name="Submit" id="Submit" value="Sihirbazı Çalıştır" />
</form>
<p><br />
</p>
</body>
...
{
$input = stripslashes($_POST['metinkutusu']);
$output = htmlspecialchars($input, ENT_QUOTES);
echo "<b>Form aracılıyla gelen XML yapısı</b><br>";
echo '<textarea name="metinkutusu" id="metinkutusu" cols="80" rows="15">';
echo $output;
echo '</textarea>';
$ArrayKeys = array();
$ArrayValues = array();
